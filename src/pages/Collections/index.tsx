  ...
  const getData = (num : number) => {
    ...
    return fetch(list[num])
      ...
      .then((responseJson) => {
        ...
        setAttributeList(attributeList => [...attributeList, responseJson.attributes]);
      })
      .catch((error) => {
        console.error(error);
      });
        ...
        <div className="collections-show-item-container">
          {
            imageList.map((imgValue, index) => {
              return(
                <div className="collections-each-container" onClick={() => {console.log(attributeList[index]); setSelectNFT(index); handleShow();}}>
                  <img className="collections-image" src={imgValue} />
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          <div style={{position: "relative"}} 
              ...
              >
            <img className="modal-image" src={imageList[selectNFT]}/>
              <div style={showDownloadIcon} className="download-icon">
                <BsDownload /> <p style={{fontSize: "20px",marginLeft:"20px", marginBottom: "0px"}}>Download Image</p>
              </div>
          </div>
          <div className="collections-show-item-container">
            {
              ...
              attributeList[selectNFT].map((attribute:any, index:any) => {
                return(
                  <div className="modal-attribute">
                    <p className="modal-attribute-font"><span className="attribute-title">Trait Type : </span><span className="attribute-value">{attribute.trait_type}</span></p>
                    <p className="modal-attribute-font"><span className="attribute-title">Value : </span><span className="attribute-value">{attribute.value}</span></p>
                  </div>
                )
              })
            }
